...
export function gitStatus(input) {
  ...
  let branch = "";
  const stagedFiles = []; const modifiedFiles = []; const untrackedFiles = [];
  let staged = 0; let modified = 0; let untracked = 0; let conflicts = 0;
  ...
  for (const raw of lines) {
    ...
    if (raw.length >= 3 && /^[ MADRCU?!][ MADRCU?!] /.test(raw)) {
      const x = raw[0]; const y = raw[1]; const file = raw.slice(3);
    ...
    if (longMatch) {
      const kind = longMatch[1]; const path = longMatch[2].trim();
